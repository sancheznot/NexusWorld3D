...
/**
 * ES: Misma superficie que MockGameRedis, respaldada por ioredis (REDIS_URL).
 * EN: Same surface as mock, backed by ioredis — multi-process / Docker / Upstash TCP.
 */
export class IoRedisGameRedis {
  constructor(
    private readonly redis: Redis,
    private readonly prefix: string
  ) {
    console.log(`✅ IoRedisGameRedis (${this.prefix})`);
  }
  ...
  async joinRoom(roomId: string, playerId: string) {
    ...
    if (rm) {
      try {
        const room = JSON.parse(rm) as { playerCount?: number };
  ...
  async getServerStats() {
    ...
    if (legacy) {
      try {
        Object.assign(out, JSON.parse(legacy) as Record<string, number>);
      } catch {
        /* ignore */
      }
    }
  ...
  async cleanupExpiredData() {
    /* ES: TTL opcional por clave en futuras versiones. EN: Optional TTLs in future. */
  }
  ...
  async savePlayerInventorySnapshot(
    ...
    try {
      ...
      console.warn("⚠️ Redis savePlayerInventorySnapshot:", e);
    }
  }
  ...
  async cleanupAllData() {
    ...
    console.log(`🧹 Redis cleared keys for prefix ${this.prefix}`);
  }
}
